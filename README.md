...
Локальный CLI-инструмент для автоматического сбора контекста кодовой базы и взаимодействия с ИИ-моделью
**в контексте проекта**.
...
- генерирует:
  ...
  - `.ai-context/context.db` — SQLite-база с контекстом проекта (вместо текстового файла).
- автоматически запускает индексацию проекта.
...
- сохраняет содержание подходящих файлв в SQLite (`context.db`).
...
Экспортирует контекст из SQLite-базы в текстовый файл (например, `out.txt`) в формате: путь/к/файлу/имя.расширение
...
| Хранение контекста       | SQLite (`context.db`) — основной формат; текстовый файл (`context.txt`) — только для экспорта/отладки |
| Упаковка                 | PyInstaller (поддержка Windows, Linux, macOS — в планах)                   |
| Локальная ИИ-модель (MVP)| `deepseek-coder:6.7b-instruct` через Ollama (`http://localhost:11434/v1`)   |
